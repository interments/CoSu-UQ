...
def build_step_keywords(step_text: str, step_probs: List[float], nlp) -> Dict[str, List[float]]:
    """
    API-free fallback keyword extraction.
    For each content word in a step, assign a single representative probability
    (the mean probability of that step).
    """
...
def build_response_keyword_payload(tokenizer, nlp, response_text: str, response_probs: List[float]):
    ...
    # Approximate step-token alignment by token count and sequential slicing.
    cursor = 0
...
if __name__ == "__main__":
    ...
    if args.use_greedy:
        suffix = f"{args.prompt_type}_greedy"
    else:
        suffix = f"{args.prompt_type}_sampled"
